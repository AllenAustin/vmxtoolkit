...
function Set-VMXNetworkAdapter {
    ...
    param(
        ...
        [Parameter(Mandatory = $false, ValueFromPipelineByPropertyName = $true)][string]$AdapterMACAddress)
    ...
    process {
        if ((get-vmx -Path $config).state -eq "stopped") {
            ...
            if ($AdapterMACAddress) {
                $Addnic += @('ethernet'+$Adapter+'.address = "'+$AdapterMACAddress+'"')
                $Addnic += @('ethernet'+$Adapter+'.addresstype = "static"')
            }
            $Content += $Addnic
            $Content | Set-Content -Path $config
            $object = New-Object -TypeName psobject
            ...
            $object | Add-Member -MemberType NoteProperty -Name Config -Value $config
            if ($AdapterMACAddress) {
                $object | Add-Member -MemberType NoteProperty -Name AdapterMACAddress -Value $AdapterMACAddress
            }
            Write-Output $object
